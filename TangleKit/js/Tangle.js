...
var Tangle = (this.Tangle = function (rootElement, modelClass) {
  ...
  function constructClass(clas, args) {
    if (typeof clas !== "function") {
      ...
      // class is constructor function, which we need to "new" with varargs (but no built-in way to do so)
      ...
      if (!ctor) {
        ...
        var ctorString =
          "(function (clas,args) { return new clas(" +
          ctorArgs.join(",") +
          "); })";
  ...
  function getFormatterForCustomFormat(formatAttribute, varNames) {
    ...
    if (varNames.length <= 1 && params.length === 0) {
      // one variable, no params
      formatter = format;
    } else if (varNames.length <= 1) {
      // one variable with params
  ...
  function addViewSettersForElement(element, varNames, view) {
    // element has a class with an update method
    var setter;
    if (varNames.length <= 1) {
  ...
  function addFormatSettersForElement(element, varNames, formatter) {
    // tangle is injecting a formatted value itself
    var span = null;
